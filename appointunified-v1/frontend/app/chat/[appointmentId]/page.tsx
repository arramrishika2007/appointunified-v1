...
export default function ChatPage() {
  ...
  useEffect(() => {
    if (hasHydrated && !isAuthenticated) {
      router.replace(`/auth/login?redirect=/chat/${appointmentId}`)
    }
  ...
  // Load chat history and refresh it on a short interval for live feel
  useEffect(() => {
    ...
    <main className="min-h-screen bg-slate-50">
      <div className="container-page max-w-7xl py-6">
        <div className="mb-6 flex flex-wrap items-center justify-between gap-4">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.2em] text-brand-600">Chat</p>
            <h1 className="mt-2 text-3xl font-bold text-slate-900">Conversation</h1>
            <p className="mt-2 max-w-2xl text-sm text-slate-500">
              Keep the booked appointment visible on the left and continue the conversation on the right.
            </p>
          </div>
              ...
              <div className="flex flex-wrap items-center justify-between gap-4">
                <div className="flex items-center gap-3">
                  ...
                  <div className="min-w-0">
                    <p className="text-xs font-semibold uppercase tracking-[0.2em] text-slate-400">Selected appointment</p>
              ...
              {!loadingMessages && messages.length === 0 && (
                <div className="flex h-full items-center justify-center py-16 text-center">
                  <div className="max-w-md">
                    ...
                    <h3 className="mt-5 text-xl font-semibold text-slate-900">Begin the conversation</h3>
                    <p className="mt-3 text-sm leading-6 text-slate-500">
                      Send the first message to start chatting. Your conversation is securely linked to this appointment.
                    </p>
                  </div>
                </div>
              )}
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className={`flex flex-col ${isMine ? 'items-end' : 'items-start'}`}>
                            <div
                              className={`max-w-xs rounded-2xl px-4 py-3 shadow-sm sm:max-w-sm lg:max-w-md ${
                                isMine
                            ...
                            >
                              {message.filtered && (
                                <div className={`mb-2 text-xs font-medium ${isMine ? 'text-brand-100' : 'text-amber-700'}`}>
                                  ⚠️ Filtered: {message.filterReason}
                                </div>
                              )}
                    ...
                    <div className="flex gap-3">
                      ...
                      <div className="rounded-2xl rounded-bl-md border border-slate-200 bg-white px-4 py-3">
                        <div className="flex items-center gap-1">
                          <span className="inline-block h-2 w-2 rounded-full bg-slate-400 animate-bounce [animation-delay:0ms]" />
                          <span className="inline-block h-2 w-2 rounded-full bg-slate-400 animate-bounce [animation-delay:150ms]" />
                          <span className="inline-block h-2 w-2 rounded-full bg-slate-400 animate-bounce [animation-delay:300ms]" />
                        </div>
                      </div>
                    </div>
                  )}
